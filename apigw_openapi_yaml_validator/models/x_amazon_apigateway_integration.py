...
class XAmazonApigatewayIntegration:
    ...
    responses: dict = field(
        default_factory=list
    )  # https://docs.aws.amazon.com/ja_jp/apigateway/latest/developerguide/api-gateway-swagger-extensions-integration-responses.html
    timeoutInMillis: int = 0
    type: str = "HTTP"
    tlsConfig: dict = field(
        default_factory=list
    )  # https://docs.aws.amazon.com/ja_jp/apigateway/latest/developerguide/api-gateway-extensions-integration-tls-config.html
    uri: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        if self.type.upper() not in ["HTTP", "AWS", "MOCK", "HTTP_PROXY", "AWS_PROXY"]:
            raise ValueError(
                "type", "value", "HTTP|VPC_LINK|AWS|MOCK|HTTP_PROXY|AWS_PROXY"
            )
        ...
        if self.contentHandling.upper() not in ["CONVERT_TO_BINARY", "CONVERT_TO_TEXT"]:
            raise ValueError(
                "contentHandling", "value", "CONVERT_TO_BINARY|CONVERT_TO_TEXT"
            )
    ...
    def __post_init__(self) -> None:
        ...
        for property in self.args.keys():
            if property.startswith("integration.request.") == False:
                raise ValueError(
                    property, "type", "integration.request."
                )  # TODO: starts with 'integration.request.'
...
class Response:
    statusCode: str = "200"
    responseTemplates: dict = field(
        default_factory=list
    )  # https://docs.aws.amazon.com/ja_jp/apigateway/latest/developerguide/api-gateway-swagger-extensions-integration-responseTemplates.html
    responseParameters: dict = field(
        default_factory=list
    )  # https://docs.aws.amazon.com/ja_jp/apigateway/latest/developerguide/api-gateway-swagger-extensions-integration-responseParameters.html
    contentHandling: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        for property in self.args.keys():
            if property.startswith("method.response.header.") == False:
                raise ValueError(
                    property, "type", "method.response.header."
                )  # TODO: starts with 'integration.request.'
